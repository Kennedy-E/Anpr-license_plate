...
pytesseract.pytesseract.tesseract_cmd = r'C:\\Program Files\\Tesseract-OCR\\tesseract.exe'  # CHANGE THIS BEFORE COMMIT


class SobelANPR:
    def __init__(self, algo, input_dir, morph, minAR=2.4, maxAR=5, debug=False, save=False):
    ...
    def debug_imshow(self, title, image,
                     waitKey=False):  # If debug argument (-d) is set to 1, the script will show the whole image processing pipeline
    ...
    def find_and_ocr(self, iteration, image, psm=7, clearBorder=False):
        ...
        if lp is not None:
            ...
            d = pytesseract.image_to_data(image, config="--psm 6", output_type=pytesseract.Output.DICT)
            n_boxes = len(d['text'])
            # text = f"{lpText[0][1]} {lpText[0][2] * 100:2f}% "
            for i in range(n_boxes):
                ...
                if int(d['conf'][i]) > 70:
                    ...
                    # OCR
                    # txt = pytesseract.image_to_string(lpText, config="--psm 6")
                    # print(txt)
